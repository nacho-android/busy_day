...
const THEME_COLORS = {
  social: ['#1a3440', '#07131f', '#8bd8c8'],
  corridor: ['#17313a', '#070d14', '#d19b57'],
  storage: ['#2d332d', '#0a1216', '#db9d4b'],
  animal: ['#28424a', '#0b1820', '#83c7af'],
  clinical: ['#1b3442', '#06101a', '#66d9dd'],
  exterior: ['#152635', '#03070d', '#ff5fa9'],
  coffee: ['#4b2c28', '#100a0b', '#ffd27a'],
} as const;
...
function buildProceduralTexture(scene: Phaser.Scene, location: LocationDefinition): string {
  ...
  wall.addColorStop(0, '#03070d');
  wall.addColorStop(.16, upper);
  wall.addColorStop(.38, lower);
  wall.addColorStop(1, '#050a10');
  context.fillStyle = wall;
  context.fillRect(0, 0, 1280, 720);

  const floor = context.createLinearGradient(0, 180, 0, 700);
  floor.addColorStop(0, `${upper}ee`);
  floor.addColorStop(.45, '#17232a');
  floor.addColorStop(1, '#0b1117');
  context.beginPath();
  context.moveTo(75, 185);
  context.lineTo(1205, 185);
  context.lineTo(1275, 700);
  context.lineTo(5, 700);
  context.closePath();
  context.fillStyle = floor;
  context.fill();

  context.strokeStyle = 'rgba(144,196,197,.10)';
  context.lineWidth = 1;
  for (let y = 235; y < 690; y += 68) {
    context.beginPath();
    context.moveTo(22, y);
    context.lineTo(1258, y);
    context.stroke();
  }
  for (let x = 90; x < 1250; x += 110) {
    context.beginPath();
    context.moveTo(640 + (x - 640) * .78, 185);
  ...
  context.lineWidth = 3;
  context.strokeRect(60, 105, 1160, 565);
  for (let index = 0; index < 9; index += 1) {
    const x = 100 + index * 135;
    const glow = context.createRadialGradient(x, 124, 2, x, 124, 95);
    glow.addColorStop(0, `${accent}68`);
    glow.addColorStop(1, `${accent}00`);
    context.fillStyle = glow;
    context.fillRect(x - 95, 30, 190, 190);
    context.fillStyle = index % 3 === 0 ? '#ff62aa' : accent;
    context.fillRect(x - 16, 111, 32, 5);
  }

  seededNoise(context, [...location.id].reduce((sum, character) => sum + character.charCodeAt(0), 0));
...
function addThemeDecor(scene: Phaser.Scene, location: LocationDefinition): void {
  ...
  if (location.theme === 'social' && !location.backgroundKey) {
    graphics.fillStyle(0x6d4c32, .9).fillRoundedRect(390, 270, 370, 190, 18)
      .lineStyle(4, 0xc49360, .22).strokeRoundedRect(390, 270, 370, 190, 18);
    for (const [x, y] of [[410, 255], [735, 255], [410, 470], [735, 470]] as const) {
      graphics.fillStyle(0xd9ded8, .65).fillRoundedRect(x, y, 50, 42, 8);
    }
  } else if (location.theme === 'storage' && !location.backgroundKey) {
    for (let shelf = 0; shelf < 5; shelf += 1) {
      const x = 250 + shelf * 185;
      graphics.fillStyle(0x263f3b, .9).fillRoundedRect(x, 122, 145, 105, 7);
      for (let box = 0; box < 3; box += 1) {
        graphics.fillStyle(0x9b784f, .62).fillRect(x + 12 + box * 41, 155, 31, 29);
      }
    }
  } else if (location.theme === 'animal' && !location.backgroundKey) {
    graphics.lineStyle(5, 0x9bb6b6, .35);
    for (const obstacle of location.obstacles.filter((item) => item.id.includes('pen') || item.id.includes('cage'))) {
      graphics.strokeRoundedRect(obstacle.x, obstacle.y, obstacle.width, obstacle.height, 8);
      for (let x = obstacle.x + 20; x < obstacle.x + obstacle.width; x += 28) {
        graphics.lineBetween(x, obstacle.y, x, obstacle.y + obstacle.height);
      }
    }
  } else if (location.theme === 'clinical' && !location.backgroundKey) {
    graphics.fillStyle(0x203747, .88);
    for (const obstacle of location.obstacles) graphics.fillRoundedRect(obstacle.x, obstacle.y, obstacle.width, obstacle.height, 10);
    graphics.lineStyle(3, 0x61ddd6, .4);
    for (const obstacle of location.obstacles) graphics.strokeRoundedRect(obstacle.x, obstacle.y, obstacle.width, obstacle.height, 10);
    ...
    for (let index = 0; index < 10; index += 1) {
      graphics.fillStyle(index % 2 ? 0x68e2d5 : 0xff61aa, .6)
        .fillCircle(170 + index * 98, 150 + Math.sin(index) * 8, 4);
    }
  }
  ...
  if (location.id === 'carPark') {
    ...
    for (let index = 0; index < 95; index += 1) {
      ...
      const y = (index * 79) % 720;
      rain.lineBetween(x, y, x - 8, y + 28);
    }
...
function addForegroundLayers(scene: Phaser.Scene, location: LocationDefinition): void {
  ...
  for (const layer of location.foregroundLayers ?? []) {
    ...
    // setCrop already offsets the quad within the source texture. Keeping the
    // image at the origin prevents a duplicated rectangular background patch.
...
export function renderBackdrop(scene: Phaser.Scene, location: LocationDefinition): void {
  if (location.backgroundKey) {
    ...
    const tintAlpha = location.id === 'carPark' ? .08 : .14;
    scene.add.rectangle(640, 360, 1280, 720, 0x03101a, tintAlpha).setDepth(1);
  } else {
  ...
  scene.add.text(78, 127, location.name.toUpperCase(), {
  ...
  scene.add.text(80, 154, location.subtitle, {
