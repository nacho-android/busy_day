...
> **Production status:** GitHub Pages serves V2.1 from `main` at merge [`d5bc627`](https://github.com/nacho-android/busy_day/commit/d5bc627235f70865290b34e8efe28824debb1e54), merged through [PR #2](https://github.com/nacho-android/busy_day/pull/2). Main workflow [`29350906888`](https://github.com/nacho-android/busy_day/actions/runs/29350906888) and [Pages job `87149283383`](https://github.com/nacho-android/busy_day/actions/runs/29350906888/job/87149283383) succeeded. The [production game](https://nacho-android.github.io/busy_day/) served `assets/index-DEBHc20z.js` and `assets/index-CyZvj29T.css`; hosted New Shift, dialogue, Tea Room HUD, refresh/Continue, and the 18-objective drawer passed with zero captured console entries. Public-control Mel/Josh completion, physical-device/controller, audio-listening, screen-reader, soak/current-phone-performance, and rights-review gates remain open.
...
- Eleven original generated retro-noir images: one title and a backplate for every gameplay location. All runtime exports are 1280×720 WebP files; their exact combined size is 1,777,598 bytes (1.695 MiB), while fourteen source-quality environment masters (including the three replacement compositions) remain available for revision.
...
- Runtime backplates are in `public/assets/backgrounds/`; all 11 are optimized 1280×720 WebP files totaling 1,777,598 bytes (1.695 MiB). Only title/Tea Room decode up front; later locations load on entry into a three-room LRU. Fourteen editable environment masters, including predecessor/replacement compositions, remain in `art/generated-masters/`.
...
- Runtime character art is fully data-driven: Mel and Josh use separate 8×4 directional sheets, and 23 named NPCs (including Vu and Shinya) use replaceable 8×4 sheets with idle, walk, and interaction phases. Animals, trolleys, vehicles, and facility equipment use their own transparent atlases; `scripts/build_runtime_art.ps1` rebuilds the browser assets from retained generated masters.
...
V2.1 PR workflow [`29348227739`](https://github.com/nacho-android/busy_day/actions/runs/29348227739) passed its clean static/unit/build gate and 48/48 dev-server plus 6/6 built-preview browser tests: 16+2 in each of Chromium, Firefox, and WebKit. After PR #2 merged, main workflow [`29350906888`](https://github.com/nacho-android/busy_day/actions/runs/29350906888) passed its quality and three browser jobs, and [Pages job `87149283383`](https://github.com/nacho-android/busy_day/actions/runs/29350906888/job/87149283383) deployed the production build. Hosted title → New Shift → dialogue → Tea Room → refresh → Continue and the objectives drawer passed with the expected hashed JS/CSS and zero captured console entries. Remaining non-CI limits are in [`docs/KNOWN_LIMITATIONS.md`](docs/KNOWN_LIMITATIONS.md).
...
The public project is [`nacho-android/busy_day`](https://github.com/nacho-android/busy_day). [PR #2](https://github.com/nacho-android/busy_day/pull/2) merged V2.1 to `main` as `d5bc627235f70865290b34e8efe28824debb1e54`; main workflow `29350906888` and Pages job `87149283383` succeeded. Pages uses workflow build type, HTTPS, and a main-only deployment policy; the workflow builds with `VITE_BASE_PATH=/busy_day/`. The V2.1 release verifier passes at 141 intended files / 50.27 MiB after exclusions, all 43 private references excluded, the V1 hash preserved, and no selected high-risk credential-pattern match. [PR #1](https://github.com/nacho-android/busy_day/pull/1) and `54499b3` remain the historical V2.0 release record.
