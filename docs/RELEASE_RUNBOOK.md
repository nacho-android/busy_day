...
The release baseline is 11 runtime backgrounds, all 1280×720 WebP, totaling exactly 1,818,586 bytes (1.734 MiB). The five retired runtime PNG copies totaled 12,006,717 bytes; conversion saved 11,180,271 bytes. All 11 source-quality 1672×941 PNG scene masters must remain in `art/generated-masters/`. The separate dialogue portrait master and non-runtime directional character source are documented independently in `docs/ASSET_MANIFEST.md`.
...
The recorded V2.0 post-optimization baseline captured 11 unique checkpoints—every room plus the ending—with zero captured console/page errors, and its contact sheet passed visual review for HUD readability, characters/targets, exits, textures, and no blank rooms. Keep `visual-qa/` ignored and out of the published release; regenerate it for V2.1 review rather than treating prior captures as permanent artefacts.

The workflow repeats release/audio/type/lint/unit/build gates and runs both the dev-server and production-preview Playwright projects for each engine. The historical V2.0 Node 24 baseline [`29297017450`](https://github.com/nacho-android/busy_day/actions/runs/29297017450) on [`bf52920`](https://github.com/nacho-android/busy_day/commit/bf52920) passed clean install, 27/27 unit tests, build, and its then-current Playwright 21/21; exact evidence is in the implementation checklist. Record a fresh workflow ID and counts for V2.1 rather than carrying this baseline forward.
...
GitHub Pages is configured with workflow build type, HTTPS, and a main-only deployment policy. `.github/workflows/quality.yml` deploys only after a successful non-PR `main` quality job and all three browser jobs; it builds with `VITE_BASE_PATH=/busy_day/` and uploads only `dist/`. The currently recorded production baseline is merge `54499b368d566f3fa4e7da1af3e7a06ed1942b2f`, successful workflow `29298026940`, successful deployment `5433749633`, and [public URL](https://nacho-android.github.io/busy_day/). Replace these identifiers only after the V2.1 main workflow and deployment finish successfully.
